...
Blockly.cake.init=function(){Blockly.cake.definitions_=Object.create(null);Blockly.cake.times_=Object.create(null);Blockly.cake.functionNames_=Object.create(null);if(Blockly.Variables){Blockly.cake.variableDB_?Blockly.cake.variableDB_.reset():Blockly.cake.variableDB_=new Blockly.Names(Blockly.cake.RESERVED_WORDS_);var a=[],b=Blockly.Variables.allVariables();Blockly.Structure.allStructure();for(var c=0;c<b.length;c++)"global"==b[c][3]&&(a[c]=b[c][0]+b[c][1]+" "+Blockly.cake.variableDB_.getName(b[c][2],
...
Blockly.cake.controls_switch=function(a){
    for(var b=0,c=Blockly.cake.valueToCode(a,"SWITCH",Blockly.cake.ORDER_NONE)||"0",d=Blockly.cake.valueToCode(a,"CASE"+b,Blockly.cake.ORDER_NONE)||b,e=Blockly.cake.statementToCode(a,"DO"+b),c="switch ("+c+") {\ncase "+d+" : \n"+e,b=1;b<=a.caseCount_;b++)
        d=Blockly.cake.valueToCode(a,"CASE"+b,Blockly.cake.ORDER_NONE)||b,e=Blockly.cake.statementToCode(a,"DO"+b),c+="\ncase "+d+" : \n"+e;
    let defaul = Blockly.cake.statementToCode(a,"DEFAULT")
    return c+"\ndefault :\n"+defaul+"}\n"
};
...
// 2018/06/07
Blockly.cake.controls_for=function(b){var a=Blockly.cake.variableDB_.getName(b.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE);if("___EC_84_A0_ED_83_9D__"==a||"--Select--"==a||"-Item-"==a)a="unselected";var e=Blockly.cake.valueToCode(b,"FROM",Blockly.cake.ORDER_ASSIGNMENT)||"0",f=Blockly.cake.valueToCode(b,"TO",Blockly.cake.ORDER_ASSIGNMENT)||"0",c=Blockly.cake.valueToCode(b,"BY",Blockly.cake.ORDER_ASSIGNMENT)||"1",d=Blockly.cake.statementToCode(b,"DO");d=Blockly.cake.addLoopTrap(d,b.id);a=(b="TRUE"==
...
c+","+a+")";break;default:throw"Unknown math operator: "+b;}Blockly.cake.definitions_.include_cake_math="#include <math.h>";return[b,Blockly.cake.ORDER_NONE]};
// Copyright 2012 Google Inc.  Apache License 2.0
Blockly.cake.procedures={};
Blockly.cake.main_block=function(c){var b=Blockly.cake.statementToCode(c,"STACK");Blockly.cake.STATEMENT_PREFIX&&(b=Blockly.cake.prefixLines(Blockly.cake.STATEMENT_PREFIX.replace(/%1/g,"'"+c.id+"'"),Blockly.cake.INDENT)+b);Blockly.cake.INFINITE_LOOP_TRAP&&(b=Blockly.cake.INFINITE_LOOP_TRAP.replace(/%1/g,"'"+c.id+"'")+b);var g=Blockly.cake.valueToCode(c,"RETURN",Blockly.cake.ORDER_NONE)||"";g=g?"  return "+g+";\n":"  return 0;\n";for(var d=[],e=[],h=[],a=0;a<c.arguments_.length;a++)d[a]=Blockly.cake.variableDB_.getName(c.arguments_[a],
...
Blockly.cake.procedures_defreturn=function(b){
    ...
    var h=Blockly.Procedures.getTypePlusArgs(b),f=[],e=[];
    for(g in Blockly.cake.times_){
        var d=Blockly.cake.times_[g];
    ...
    "pointer"==g?(g=b.getFieldValue("PSPECS"),null==g&&(g="*"),a=f+g+" "+c+"("+h.join(", ")+") {"+e.replace(/\n\n+/g,"\n\n").replace(/\n*$/,"\n")+a+"}"):"array"==g?(g=b.getFieldValue("ASPECS"),null==g&&(g="[]"),a=f+g+" "+c+"("+h.join(", ")+") {"+e.replace(/\n\n+/g,"\n\n").replace(/\n*$/,"\n")+a+"}"):a=f+" "+c+"("+h.join(", ")+") {"+e.replace(/\n\n+/g,"\n\n").replace(/\n*$/,"\n")+a+"}";
    a=Blockly.cake.scrub_(b,a);
    Blockly.cake.definitions_[c]=a;
    Blockly.cake.definitions_["Func_declare"+c]=f+" "+c+"("+h.join(", ")+");";
    -1==Blockly.Blocks.checkLegalName(Blockly.Msg.PROCEDURES_ILLEGALNAME,c)&&this.initName();
    return null
};
Blockly.cake.procedures_defnoreturn=function(b){var c=Blockly.cake.variableDB_.getName(b.getFieldValue("NAME"),Blockly.Procedures.NAME_TYPE),a=Blockly.cake.statementToCode(b,"STACK"),h=[],f=[],e;for(e in Blockly.cake.times_){var d=Blockly.cake.times_[e];e.match("srand")?d[0]==c&&(d=Blockly.cake.prefixLines(d[1],Blockly.cake.INDENT),h.push(d)):e.match("time")&&d[0]==c&&(d=Blockly.cake.prefixLines(d[1],Blockly.cake.INDENT),f.push(d))}f=h.length?h.join("\n")+"\n"+f.join("\n"):f.join("\n");Blockly.cake.STATEMENT_PREFIX&&
...
Blockly.cake.library_stdio_printf= function(p) {
    ...
    for (var l = 0; l <= p.printAddCount_; l++) {
        var separator = (l === 0) ? "" : " << ";
        ...
        if (e) {
            ...
            switch (a) {
                ...
                case "library_string_strcat":
                case "library_string_strcpy":
                case "library_string_strcmp":
                case "library_math_abs":
                case "variables_array_get":
                case "variables_array_pointer":
                case "logic_operation":
                case "logic_compare":
                    output += separator + b;
                    break;
                // Caso para caracteres
                case "library_stdio_text_char":
                    output += separator + b;
                    break;
                // Casos para strings
                case "library_stdlib_convert":
                case "library_stdio_text":
                    output += separator + '"' + b + '"';
                    break;
                case "library_stdio_newLine":
                    output += separator + "endl";
                    break;
                default:
                    alert("Reporta este error a Isaí: " + a);
                    break;
            }
        }
    }

    Blockly.cake.definitions_.include_cplusplus_iostream = "#include <iostream>\nusing namespace std;";
    return output + ";\n"; 
};
//2018/06/20
...
Blockly.cake.library_stdio_scanf=function(g){
    ...
    for(h=0; h<=g.scanAddCount_; h++){
        ...
        if(f) {
            ...
            } else if(["variables_pointer_get", "variables_pointer_&", "variables_pointer_*"].includes(a)) {
                inputStr += " >> " + b;
            } else {
                inputStr += " >> " + b;
            }
...
Blockly.cake.arrTypeCheckInScan2=function(f,b){for(var a="",c=b.targetBlock(),d=Blockly.Blocks.getWantedBlockArray("a"),e=0;e<d.length;e++){var g=Blockly.FieldDropdown.prototype.getTypefromVars(d[e][2],0),h=d[e][5][0],k=c.getInputIdxLength();if(h==k){"int"==g?a="%d":"unsigned int"==g?a="%u":"float"==g?a="%f":"double"==g?a="%lf":"char"==g?a="%c":"dbchar"==g&&(a="%s");break}else if(h>k&&"char"==d[e][0]){a="%s";break}else b.isSuperior()?b.targetBlock().setParent(null):b.sourceBlock_.setParent(null),
b.sourceBlock_.bumpNeighbours_()}return a};
...
Blockly.cake.structure_define=function(a){for(var c=Blockly.cake.variableDB_.getName(a.getFieldValue("NAME"),null),d=[],e=[],f=[],h=[],g=[],b=0;b<a.members_.length;b++)d[b]=Blockly.cake.variableDB_.getName(a.members_[b],Blockly.Variables.NAME_TYPE),e[b]=a.types_[b],f[b]=a.dist_[b],h[b]=a.spec_[b],"v"==f[b]?g[b]="   "+e[b]+" "+d[b]+";\n":"a"==f[b]?g[b]="   "+e[b]+" "+d[b]+"["+h[b]+"];\n":"p"==f[b]&&(g[b]="   "+e[b]+" "+h[b]+d[b]+";\n");a="typedef struct\n{\n"+g.join("")+"} "+c+";\n";-1==Blockly.Blocks.checkLegalName(Blockly.Msg.STRUCTURE_ILLEGALNAME,
c)&&this.initName();return a};Blockly.cake.structure_declare=function(a){var c=Blockly.cake.variableDB_.getName(a.getFieldValue("TYPES"),null);c=Blockly.Blocks.checkUnselect(c);a=Blockly.cake.variableDB_.getName(a.getFieldValue("NAME"),Blockly.Variables.NAME_TYPE);-1==Blockly.Blocks.checkLegalName(Blockly.Msg.STRUCTURE_ILLEGALNAME,a)&&this.initName();return c+" "+a+";\n"};
...
".slice(-"+c+").charAt(0)",Blockly.cake.ORDER_FUNCTION_CALL];case "RANDOM":return b=Blockly.cake.provideFunction_("text_random_letter",["function "+Blockly.cake.FUNCTION_NAME_PLACEHOLDER_+"(text) {","  var x = Math.floor(Math.random() * text.length);","  return text[x];","}"])+"("+a+")",[b,Blockly.cake.ORDER_FUNCTION_CALL]}throw"Unhandled option (text_charAt).";};
...
Blockly.cake.variables_array_get=function(a){var e=Blockly.cake.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE);e=Blockly.Blocks.checkUnselect(e);var f=Blockly.cake.valueToCode(a,"LENGTH_1",Blockly.cake.ORDER_NONE)||"-1",h=Blockly.cake.valueToCode(a,"LENGTH_2",Blockly.cake.ORDER_NONE)||"-1",k=Blockly.cake.valueToCode(a,"LENGTH_3",Blockly.cake.ORDER_NONE)||"-1",g=Blockly.Blocks.getWantedBlockArray("a"),b=Blockly.Blocks.getIndexArray(g,e);g="";var c=Blockly.Blocks.checkArrayIndex(f,
...
Blockly.cake.variables_array_set=function(a){var e=Blockly.cake.valueToCode(a,"VALUE",Blockly.cake.ORDER_ASSIGNMENT)||"0",f=Blockly.cake.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE);f=Blockly.Blocks.checkUnselect(f);var h=Blockly.cake.valueToCode(a,"LENGTH_1",Blockly.cake.ORDER_NONE)||"-1",k=Blockly.cake.valueToCode(a,"LENGTH_2",Blockly.cake.ORDER_NONE)||"-1",g=Blockly.cake.valueToCode(a,"LENGTH_3",Blockly.cake.ORDER_NONE)||"-1",b=Blockly.Blocks.getWantedBlockArray("a"),
...
Blockly.cake.variables_matrix_declare=function(a){
...
Blockly.cake.variables_cubo_declare=function(a){
    var g=Blockly.cake.valueToCode(a,"VALUE",Blockly.cake.ORDER_ASSIGNMENT)||"0",
        d=Blockly.cake.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE),
        f=a.getFieldValue("TYPES"),
        b=Blockly.cake.valueToCode(a,"LENGTH_1",Blockly.cake.ORDER_NONE)||"0",
